...
public class ExtraUtils {
    ...
    public static boolean isStore(String input) {
        if (input.length() == 7) {
            if (input.substring(0, 5).equals("STORE")) {
                if (Character.isLowerCase(input.charAt(6))) {
                    return true;
                } // end if
            } // end if
        } // end if
    ...
    public static boolean isOperation(String input) {
        return (input.length() == 1 && ((input.charAt(0) == '+' || input.charAt(0) == '-' || input.charAt(0) == '*' || input.charAt(0) == '/')));
    } // end method
     ...
     * @param char the opperation being run. defaults to 'a'
     * @return boolean wether or not the given data was a valid call which got
     * run.
     *
     */
    public static boolean calcRunner(String formula, BFCalculator[] calcs, BFRegisterSet register, int i, char operation) {
